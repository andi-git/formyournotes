...
public class ActionBarHelperBase extends ActionBarHelper {
	...
	private final ActionBarR actionBarR;

	protected Set<Integer> mActionItemIds = new HashSet<Integer>();

	protected ActionBarHelperBase(Activity activity, ActionBarR actionBarR) {
		super(activity);
		this.actionBarR = actionBarR;
	}
	...
	public void onPostCreate(Bundle savedInstanceState) {
		mActivity.getWindow().setFeatureInt(Window.FEATURE_CUSTOM_TITLE,
				actionBarR.getLayout().getActionbarCompat());
		setupActionBar();
	...
	private void setupActionBar() {
		...
		SimpleMenuItem homeItem = new SimpleMenuItem(tempMenu,
				android.R.id.home, 0, mActivity.getString(actionBarR
						.getStirng().getAppName()));
		homeItem.setIcon(actionBarR.getDrawable().getIcHome());
		addActionItemCompatFromMenuItem(homeItem);

		// Add title text
		TextView titleText = new TextView(mActivity, null, actionBarR
				.getAttribute().getActionbarCompatTitleStyle());
	...
	public void setRefreshActionItemState(boolean refreshing) {
		View refreshButton = mActivity.findViewById(actionBarR.getId()
				.getActionbarCompatItemRefresh());
		View refreshIndicator = mActivity.findViewById(actionBarR.getId()
				.getActionbarCompatItemRefreshProgress());
	...
	protected void onTitleChanged(CharSequence title, int color) {
		TextView titleView = (TextView) mActivity.findViewById(actionBarR
				.getId().getActionbarCompatTitle());
		if (titleView != null) {
	...
	private ViewGroup getActionBarCompat() {
		return (ViewGroup) mActivity.findViewById(actionBarR.getId()
				.getActionbarCompat());
	}
	...
	private View addActionItemCompatFromMenuItem(final MenuItem item) {
		...
		ImageButton actionButton = new ImageButton(mActivity, null,
				itemId == android.R.id.home ? actionBarR.getAttribute()
						.getActionbarCompatItemHomeStyle() : actionBarR
						.getAttribute().getActionbarCompatItemStyle());
		actionButton.setLayoutParams(new ViewGroup.LayoutParams((int) mActivity
				.getResources().getDimension(
						itemId == android.R.id.home ? actionBarR.getDimension()
								.getActionbarCompatButtonHomeWidth()
								: actionBarR.getDimension()
										.getActionbarCompatButtonWidth()),
				ViewGroup.LayoutParams.FILL_PARENT));
		if (itemId == actionBarR.getId().getMenuRefresh()) {
			actionButton.setId(actionBarR.getId()
					.getActionbarCompatItemRefresh());
		}
		...
		if (item.getItemId() == actionBarR.getId().getMenuRefresh()) {
			// Refresh buttons should be stateful, and allow for indeterminate
			// progress indicators,
			// so add those.
			ProgressBar indicator = new ProgressBar(mActivity, null, actionBarR
					.getAttribute().getActionbarCompatProgressIndicatorStyle());
			...
			indicator.setId(actionBarR.getId()
					.getActionbarCompatItemRefreshProgress());
