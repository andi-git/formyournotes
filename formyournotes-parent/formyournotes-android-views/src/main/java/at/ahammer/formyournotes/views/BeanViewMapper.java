...
public class BeanViewMapper {
	...
	public View getView(Context context, MyR r, FormBean formBean, FormYourNotesBean<?> currentBean) {
		if (currentBean instanceof EditTextBean) {
			return new EditTextView(context, r, formBean, (EditTextBean) currentBean);
		} else if (currentBean instanceof CheckBoxGroupBean) {
			return new CheckBoxGroupView(context, r, formBean, (CheckBoxGroupBean) currentBean);
		} else if (currentBean instanceof CheckBoxBean) {
			return new CheckBoxView(context, r, formBean, (CheckBoxBean) currentBean);
		} else if (currentBean instanceof ContactBean) {
			return new ContactView(context, r, formBean, (ContactBean) currentBean);
